...
async def createOTP(request: Request, response: Response, data: DecryptRequest, db: Session = Depends(get_db)):
    try:
        ...
        if not pp["status_code"] == 200:
            ...
            return pp
        pay = pp["payload"]
        oold = db.query(OTP).filter(OTP.countryCode == pay["countryCode"], OTP.phoneNumber == pay["phoneNumber"],
                                    OTP.status == "pending").first()
        if not oold is None:
            db.query(OTP).filter(OTP.countryCode == pay["countryCode"], OTP.phoneNumber == pay["phoneNumber"],
                                 OTP.status == "pending").update({"status": "expired"})
        o = OTP(dateTime=datetime.now(), phoneNumber=pay["phoneNumber"], countryCode=pay["countryCode"],
                otp=generateOTP(OTPLength), status="pending", remainingTrials=numberOfOPTTrials,
                timeoutTime=datetime.now() + timedelta(minutes=OTPvalidMinutes))
        db.add(o)
        db.commit()
...
async def checkOTP(request: Request, response: Response, data: DecryptRequest, db: Session = Depends(get_db)):
    try:
        ...
        if not pp["status_code"] == 200:
            ...
            return pp
        pay = pp["payload"]
        o = db.query(OTP).filter(OTP.countryCode == pay["countryCode"], OTP.phoneNumber == pay["phoneNumber"],
                                 OTP.status == "pending").first()
        if o is None:
            log("error",
                "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /checkOTP body: " + str(
                    pay) + " response: 401 no otp waiting for this number")
            return {"status_code": 401, "message": "no otp waiting for this number"}
        elif o.status == "expired":
            log("error",
                "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /checkOTP body: " + str(
                    pay) + " response: 401 otp expired please create a new one")
            ...
            if o.remainingTrials == 1:
                db.query(OTP).filter(OTP.id == o.id).update(
                    {"remainingTrials": o.remainingTrials - 1, "status": "expired"})
                db.commit()
                log("error",
                    "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /checkOTP body: " + str(
                        pay) + " response: 402 incorrect OTP, trials expired")
            ...
            log("error",
                "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /checkOTP body: " + str(
                    pay) + " response: 402 incorrect OTP, " + str(o.remainingTrials) + " trials remaining")
            return {"status_code": 402, "message": "incorrect OTP", "trials remaining": o.remainingTrials}
        ...
        log("error",
            "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /checkOTP response: " + str(e))
        return {"status_code": 401, "message": message}
...
async def createCustomer(request: Request, response: Response, data: DecryptRequest,
    ...
    try:
        ...
        cus = db.query(Customer).filter(Customer.countryCode == pay["countryCode"],
                                        Customer.phoneNumber == pay["phoneNumber"],
                                        not Customer.status == "inactive").first()
        if not cus is None:
            log("error",
                "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /createCustomer body: " + str(
                    pay) + " response: 401 phone number is taken")
            return {"status_code": 401, "message": "this phone number is taken"}

        oold = db.query(OTP).filter(OTP.countryCode == pay["countryCode"], OTP.phoneNumber == pay["phoneNumber"],
                                    OTP.status == "complete").first()
        if oold is None:
            log("error",
                "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /createCustomer body: " + str(
                    pay) + " response: 401 phone number not confirmed")
        ...
        if datetime.now() < bDate:
            log("error",
                "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /createCustomer body: " + str(
                    pay) + " response: 402 less than 18 years")
            return {"status_code": 402, "message": "you have to be older than 18 years to sign up"}

        c = Customer(dateTime=datetime.now(), phoneNumber=pay["phoneNumber"], countryCode=pay["countryCode"],
                     nationalInterID=pay["ID/Iqama"], birthDate=pay["DOB"], status="pending",
                     firstName=pay["firstName"], lastName=pay["lastName"])
        if "email" in pay:
            c.email = pay["email"]
        db.add(c)
        db.commit()
        ...
        log("error",
            "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /createCustomer response: " + str(
                e))
        return {"status_code": 401, "message": message}
...
async def verifyCustomer(request: Request, response: Response, data: DecryptRequest,
    ...
    # try:
        ...
        if  cus is None:
            ...
            log("error", "IP: " + request.client.host + " time: " + str(
                datetime.now()) + " api: /verifyingCustomer body: " + str(pay) + " response: 401 customer inactive")
        ...
        if cus.status == "pending":
            if not True:  # FOCAL SANCTION LISTING
                log("error", "IP: " + request.client.host + " time: " + str(
                    datetime.now()) + " api: /verifyingCustomer body: " + str(
                    pay) + " response: 301 customer await compliance team response (Focal Sanction)")
                return {"status_code": 301,
                        "message": "your request is being reviewed, this might take up to one working day"}
        ...
        if cus.status == "Fapproved":
            if not True:  # risk assessment
                log("error", "IP: " + request.client.host + " time: " + str(
                    datetime.now()) + " api: /verifyingCustomer body: " + str(
                    pay) + " response: 301 customer await compliance team response (risk assessment)")
                return {"status_code": 301,
                        "message": "your request is being reviewed, this might take up to one working day"}
...
async def createAccount(request: Request, response: Response, data: DecryptRequest, db: Session = Depends(get_db)):
    try:
        ...
        if  cus is None:
            ...
            log("error",
                "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /createAccount body: " + str(
                    pay) + " response: 401 customer inactive")
            return {"status_code": 401, "message": "no active customer has this ID"}

        if not cus.status == "approved":
            log("error",
                "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /createAccount body: " + str(
                    pay) + " response: 401 customer not fully verified")
        ...
        message = "exception " + str(e) + " occurred with creating account"
        log("error",
            "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /createAccount response: " + str(e))
        return {"status_code": 401, "message": message}
...
async def addKYC(request: Request, response: Response, data: DecryptRequest, db: Session = Depends(get_db)):
    try:
        ...
        log("error",
            "IP: " + request.client.host + " time: " + str(datetime.now()) + " api: /addKYC response: " + str(e))
        return {"status_code": 401, "message": message}
